...
async def _run_pipeline(
    ...
    if is_realtime:
        ...
        tts = create_tts_service(user_config, audio_config, run_configs=run_configs)
    ...
    # Apply per-workflow VAD config if present (BDR feature)
    vad_config = run_configs.get("vad_config")
    if vad_config:
        def _clamp(value, low, high, name):
            if value < low:
                logger.warning(f"vad_config.{name}={value} below minimum {low}, clamping")
                return low
            if value > high:
                logger.warning(f"vad_config.{name}={value} above maximum {high}, clamping")
                return high
            return value

        user_vad_analyzer = SileroVADAnalyzer(params=VADParams(
            confidence=_clamp(vad_config.get("confidence", 0.5), 0.0, 1.0, "confidence"),
            start_secs=_clamp(vad_config.get("start_secs", 0.2), 0.0, 10.0, "start_secs"),
            stop_secs=_clamp(vad_config.get("stop_secs", 0.8), 0.0, 10.0, "stop_secs"),
            min_volume=_clamp(vad_config.get("min_volume", 0.6), 0.0, 1.0, "min_volume"),
        ))
    else:
    ...
    # ── Goodbye phrase detection (BDR feature) ────────────────────────────
    goodbye_detector = None
    goodbye_config = run_configs.get("goodbye_detection", {})
    if goodbye_config.get("enabled", False) and not is_realtime:
        phrases = goodbye_config.get("phrases", [])
        if phrases:
            from api.services.pipecat.goodbye_detector import GoodbyeDetectorProcessor

            goodbye_detector = GoodbyeDetectorProcessor(
                phrases=phrases,
                action=goodbye_config.get("action", "end_call"),
            )
            logger.info(
                f"Goodbye detection enabled with {len(phrases)} phrases "
                f"for workflow run {workflow_run_id}"
            )

    # ── Call state detection (BDR feature) ────────────────────────────────
    call_state_detector = None
    call_state_config = run_configs.get("call_state_detection", {})
    if call_state_config.get("enabled", False) and not is_realtime:
        from api.services.pipecat.call_state_detector import CallStateDetectorProcessor

        # Resolve LLM for classification
        if call_state_config.get("use_workflow_llm", False):
            classification_llm = create_llm_service(user_config)
        else:
            classification_llm = create_llm_service_from_provider(
                provider=call_state_config.get("provider", "groq"),
                model=call_state_config.get("model", "llama-3.3-70b-versatile"),
                api_key=call_state_config.get("api_key", ""),
            )

        call_state_detector = CallStateDetectorProcessor(
            llm_service=classification_llm,
            max_ivr_secs=call_state_config.get("max_ivr_secs", 60),
            ivr_target_description=call_state_config.get("ivr_target_description", ""),
            voicemail_message=call_state_config.get("voicemail_message"),
        )
        logger.info(f"Call state detection enabled for workflow run {workflow_run_id}")
    ...
    if is_realtime:
        ...
        pipeline = build_pipeline(
            ...
            call_state_detector=call_state_detector,
        )
    ...
    # ── Callback scheduling tool registration (BDR feature) ───────────────
    callback_config = run_configs.get("callback_scheduling", {})
    if callback_config.get("enabled", False) and not is_realtime:
        from api.services.callbacks.tool import (
            SCHEDULE_CALLBACK_SCHEMA,
            handle_schedule_callback,
        )

        # Extract context for the callback handler
        initial_context = merged_call_context_vars or {}
        cb_org_id = workflow.organization_id
        cb_campaign_id = initial_context.get("campaign_id")
        cb_phone = initial_context.get("phone_number", "")
        cb_lead_name = initial_context.get("lead_name")
        cb_company = initial_context.get("company")

        async def _schedule_callback_handler(function_name, tool_call_id, args, llm, context, result_callback):
            result = await handle_schedule_callback(
                args=args,
                organization_id=cb_org_id,
                workflow_run_id=workflow_run_id,
                campaign_id=cb_campaign_id,
                phone_number=cb_phone,
                lead_name=cb_lead_name,
                company=cb_company,
            )
            await result_callback(result)

        try:
            engine.llm.register_function(
                "schedule_callback", _schedule_callback_handler
            )
            logger.info(f"Callback scheduling tool registered for workflow run {workflow_run_id}")
        except Exception as e:
            logger.warning(f"Failed to register callback scheduling tool: {e}")
