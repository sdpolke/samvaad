...
def build_pipeline(
    ...
    Args:
        ...
        goodbye_detector: Optional GoodbyeDetectorProcessor. When provided,
            inserts after STT (and after voicemail detector if present) to detect
            goodbye phrases and terminate calls.
        call_state_detector: Optional CallStateDetectorProcessor. When provided,
            inserts after STT for outbound calls to detect IVR/voicemail/human.
    """
    ...
    # Insert call state detector after STT (and after voicemail detector)
    if call_state_detector:
        logger.info("Adding call state detector to pipeline")
        processors.append(call_state_detector)

    # Insert goodbye detector after STT (and after voicemail/call-state detectors)
    if goodbye_detector:
        logger.info("Adding goodbye detector to pipeline")
        processors.append(goodbye_detector)
