...
def create_tts_service(user_config, audio_config: "AudioConfig", run_configs: dict | None = None):
    ...
    Args:
        ...
        audio_config: Audio configuration for sample rates
        run_configs: Optional workflow_configurations dict for per-workflow overrides
    """
    logger.info(
        f"Creating TTS service: provider={user_config.tts.provider}, model={user_config.tts.model}"
    )
    # Warn if tts_expression is configured but provider is not Cartesia
    tts_expression_configured = bool((run_configs or {}).get("tts_expression"))
    if tts_expression_configured and user_config.tts.provider != ServiceProviders.CARTESIA.value:
        logger.warning(
            f"tts_expression configured but TTS provider is '{user_config.tts.provider}', "
            "not Cartesia. tts_expression will be ignored."
        )
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
        ...
        # Apply tts_expression from workflow_configurations (BDR feature)
        tts_expression = (run_configs or {}).get("tts_expression")
        if tts_expression:
            expr_speed = tts_expression.get("speed")
            expr_emotion = tts_expression.get("emotion")
            if expr_speed and expr_speed != 1.0:
                gen_config_kwargs["speed"] = expr_speed
            if expr_emotion:
                gen_config_kwargs["emotion"] = [expr_emotion]
