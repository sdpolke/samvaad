...
Whenever the codebase changes — a `git pull` or a local edit — rebuild the `api` and `ui` images and recreate the running containers so they pick up the new code.
