...
For CRM integrations (e.g. [Twenty CRM](/integrations/twenty/overview)), send `X-Idempotency-Key` or `idempotency_key` in the body to prevent duplicate calls when the external system retries. Repeated requests return `status: duplicate` with the original `workflow_run_id`.

Send `X-Correlation-Id` to store an external trace ID in `initial_context.external_correlation_id` for post-call webhook correlation.
