...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isSamvaadDomain = window.location.hostname.endsWith('.samvaad.com');
  const cookieDomainPart = isSamvaadDomain ? '; domain=.samvaad.com' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.samvaad.com). The route will set the
  ...
  // superadmin.samvaad.com we want to switch to app.samvaad.com. For any other domain
